...
if [ 0 -eq $(docker network ls | grep -c $user_created_network_name) ] ; then
    ...
    docker network create --driver bridge $user_created_network_name
fi
...
echo "gitlab [ $(docker run -d --rm --name gitlab --network $user_created_network_name --network-alias gitlab -e GITLAB_ROOT_PASSWORD='erikliberal@infoxx2017!' -e GITLAB_ROOT_EMAIL='erik.liberal@gmail.com' -v gitlab-data:/var/opt/gitlab gitlab/gitlab-ce:9.4.5-ce.0) ] on ip < $(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' gitlab) >"
...
echo "nexus [ $(docker run -d --rm --name nexus --network $user_created_network_name --network-alias nexus -v nexus-data:/nexus-data sonatype/nexus3) ] on ip < $(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' nexus) >"
...
echo "jenkins [ $( docker run -d --rm --name=jenkins --network $user_created_network_name --network-alias jenkins -v jenkins-data:/var/jenkins_home jenkins/jenkins:2.60.2 ) ] on ip < $(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' jenkins) >"
